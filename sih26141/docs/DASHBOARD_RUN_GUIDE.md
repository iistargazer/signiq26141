...
**35 tests total, 0 failed**. This is also great evidence if a judge
...
- Dark dashboard titled **"Quantum-Secured Pipeline ⚛"**
- Top-right pill: **● API online** (green)
- Panels in order: Experiment Parameters → status cards → Live Channel
  Monitor → QBER vs. Eavesdropping Intensity → **⚛ QDS Signature Lab**
  (with the ①–⑤ numbered buttons) → footer.

### 2.2 The 5-click smoke test (2 minutes, do it before judges arrive)
...
| **Run parameter sweep** | sweep panel | Bars track the dark theory bars; red dashed threshold line crossed around 50–55% |
| **1 · Generate quantum keys** | QDS Lab | Green chip "P(forgery) < 10^-39" appears |
| **2 · Sign via teleportation** | QDS Lab | Green chip "✓ Bob verified · 1-ACC (transferable)" + teleportation trace table |
| **3 · Launch all 5 attacks** | QDS Lab | All 5 rows red-free: every attack shows "✓ rejected" |

If all five behave, you're demo-ready. (Buttons 4/5 — forgery analysis
and performance evaluation — also work; each takes ~1–3 s.)
