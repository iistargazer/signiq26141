...
pub fn recommended_thresholds(basis_misalignment: f64) -> Thresholds {
    Thresholds {
        c1: (SIX_STATE_NOISE_FLOOR + basis_misalignment).min(0.25),
        c2: 0.25,
    }
}
...
mod tests {
    ...
    fn tampered_channel_raises_mismatch_above_noise_floor() {
        ...
        let p = params(1200);
        // 30% disturbance: expected mismatch ≈ 0.375 — above the 0.25
        // rejection threshold but below the ≈0.5 guessing floor, so the
